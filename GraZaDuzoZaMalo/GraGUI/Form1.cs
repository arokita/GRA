...

{
...

    {
        ...
        private void statystyki()
        {
            ...
            labelLiczbaRuchow.Text = "Liczba ruchów= " + gra.Historia.Count.ToString();
            TimeSpan czas = gra.Historia[gra.Historia.Count - 1].Czas - gra.Historia[0].Czas;
            labelCzasGry.Text = $"Czas gry = {czas}";

        }

        private void buttonWyśłij_Click(object sender, EventArgs e)
        {
            int propozycja = int.Parse(textBoxPropozycja.Text);
            var odpowiedz = gra.Ocena(propozycja);                      // logika może być dwuwartosciowa:false,true; trójwartościowa: false,true,null; logika rozmyta:prawdopodobieństwo
            switch (odpowiedz)
            {
